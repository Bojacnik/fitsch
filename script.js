...
let subjects = []; // Array of selected subjects
let lastLoadedSubjects = []; // Array of last loaded subjects
let ranges = []; // Array of ranges of selected subjects
let lessons = []; // Array of lessons of selected subjects
let file = {
    "sem": "", "studies": [], "grades": [],
    "subjects": [], "custom": [], "selected": [], "deleted": []
};                                // File cache
let year = (new Date()).getMonth() + 1 >= 8 ? (new Date()).getFullYear() : (new Date()).getFullYear() - 1; // Academic year (from August display next ac. year)
const fakeHtml = document.implementation.createHTMLDocument('virtual'); // https://stackoverflow.com/a/50194774/7361496
const loadUrl = "./load.php";
///////////////////////////////////// Main /////////////////////////////////////
$(document).ready(async function() {
    debugger
    // Semester radio auto select
    const today = new Date();
    if(today.getMonth() === 11 || today.getMonth() < 4) {
...
$.ajax({
    ...
    method: "GET",
    async: true,
    success: async function(sourceCode) {
        ...
        console.log(pyodide.runPython(sourceCode));
    }
});
...
// Menu
$(document).on("change", ".year_select", async function() {
...
$(document).on("click", ".menu_sem_radio", function() {
    let menu_com_search_input = $(".menu_com_search_input")
...
$(document).on("click", ".menu_mit_radio", function() {
    ...
    let menu_opt_search_input = $(".menu_opt_search_input")
...
$(document).on("keyup", ".menu_com_search_input", function() {
    let menu_com_column_and_row = $(".menu_com_column .menu_column_row")
    .removeClass("hidden_search");
    if($(".menu_com_search_input").prop("value") !== "") {
        menu_com_column_and_row.addClass("hidden_search");
        menu_com_column_and_row.each(function(i, sub) {
...
$(document).on("keyup", ".menu_opt_search_input", function() {
    ...
    menu_com_column_and_row.removeClass("hidden_search");
...
$(document).on("click", ".schedule_cell_star", function() {
    // Update
    const les = lessons.find(x => x.id === $(this).siblings(".id").html());
    les.selected = les.selected !== true;
...
$(document).on("click", ".schedule_cell_bin", function() {
    // Update
    const lesson = lessons.find(x => x.id === $(this).siblings(".id").html());
    if(lesson.type === "custom") {
        lessons = lessons.filter(x => x.id !== $(this).siblings(".id").html());
    } else {
        lesson.deleted = lesson.deleted !== true;
    }
...
$(document).on("click", ".lesson_add_card_button", function() {
    // New lesson
    let lesson_add_card_name = $(".lesson_add_card_name")
    const lesson = {
...
async function loadStudies() {
    // Title
    let loading_message = $(".loading_message")
    loading_message.html("Načítám studia...");

    // AJAX
    studies = [];
    try {
        const studiesData = await $.ajax({
            url: loadUrl,
            method: "POST",
            data: {
                "a": "s",
        ...
        // Generate
        let menu_column_row_select = $(".menu_column_row_select")
...
function renderSubjects() {
    ...
    let menu_mit_radio_checked = $(".menu_mit_radio:checked")
    if(menu_mit_radio_checked.length > 0) {
    ...
    // Render groups
    const groups = [];
    let bitSelected = false;
    let mitSelected = false;
    ...
    if(mitSelected) {
        groups.push("2_" + menu_mit_radio_checked.prop("value"));
    }
...
function loadLessons() {
    ...
    // Make file
    let menu_sel_checkbox = $(".menu_sel_checkbox")
    {
        // Year
        file.year = year;
        ...
        let menu_mit_radio_checked = $(".menu_bit_checkbox:checked")
        if(menu_mit_radio_checked.length > 0) {
            file.studies.push(menu_mit_radio_checked.prop("value"));
        }
        if(menu_mit_radio_checked.length > 0) {
            file.studies.push(menu_mit_radio_checked.prop("value"));
        }
    ...
    $.each(subjects, function(i, sub) {
        ...
        const blueLesson = lessons.find(x => x.name === sub.name && x.type === "blue");
        if(typeof blueLesson != "undefined" && blueRange > 0) {
            blueLength = blueLesson.to - blueLesson.from;
        }
        const yellowLesson = lessons.find(x => x.name === sub.name && x.type === "yellow");
...
function renderAll() {
    ...
    $.each(lessons, function(i, lesson) {
        ...
        const key = lesson.name + ";" + lesson.day + ";" + lesson.from + ";" + lesson.to + ";" + lesson.type;
    ...
    $.each(lessonsDisection, function(i, lessonsDisection) {
        ...
        const oddLessons = lessonsDisection.filter(x => isOddWeek(x.week, 1));
        const evenLessons = lessonsDisection.filter(x => isEvenWeek(x.week, 1));
        const otherLessons = lessonsDisection.filter(x => !isOddWeek(x.week, 1) && !isEvenWeek(x.week, 1));
...
// Reinit
        ...
        do {
            collision = false;
            $.each(schedule[lesson.day], function(o, lesX) {
                if(lesson.layer === lesX.layer) {
                    if(doLessonsCollide(lesson.from, lesson.to, lesX.from, lesX.to)) {
                        collision = true;
                        lesson.layer++;
                    }
                }
    ...
    // Layers count
    const scheduleLayersCount = [1, 1, 1, 1, 1];
    for(let d = 0; d < 5; d++) {
        let maxLayer = 1;
    ...
    // Generation of cells
    for(let d = 0; d < 5; d++) {
        const fullLength = +$(".schedule_all").find(".schedule_row").eq(d).children(".schedule_row_layers").width();
        ...
        $.each(schedule[d], function(i, les) {
            ...
            const layerDiv = $(".schedule_all").find(".schedule_row").eq(d).children(".schedule_row_layers").children(".schedule_row_layer").eq(les.layer - 1);
...
function renderScheduleFin() {
    ...
    // Generation of cells
    for(let d = 0; d < 5; d++) {
        const fullLength = +schedule_fin.find(".schedule_row").eq(d).children(".schedule_row_layers").width();
        ...
        $.each(schedule[d], function(i, les) {
            ...
            const layerDiv = $(".schedule_fin").find(".schedule_row").eq(d).children(".schedule_row_layers").children(".schedule_row_layer").eq(les.layer - 1);
...
function loadJSON() {
    // No file
    let json_load_input = $(".json_load_input");
...
function getLessonCategory(lesson) {
    const isCustom = lesson.type === "custom";
    const color = isCustom ? lesson.custom_color : lesson.type;
    let out = "";
...
function exportICal() {
    let contents = "";
    const createdDatetime = getIcalDatetime(new Date);
    ...
    $.each(lessons, function(j, les) {
        ...
        const fromDatetime = getDatetimeFromHourNumber(les.from, les.day, les.week);
        const fromDatetimeIcal = getIcalDatetime(fromDatetime);
        let toDatetime = getDatetimeFromHourNumber(les.to, les.day, les.week);
        toDatetime = new Date(toDatetime.getTime() - 10 * 1000 * 60);
        const toDatetimeIcal = getIcalDatetime(toDatetime);

        const typeString = getLessonTypeByColorAsString(les.type);
...
function parseWeek(week) {
    ...
    if(week.match(/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/)) {
        const weekNum = getSemesterWeekFromDate(new Date(week));
...
function getLessonTypeByColorAsString(type) {
    switch(type) {
        case "green":
            return "Přednáška";
        case "blue":
...
function getSemesterWeekFromDate(date) {
    ...
    const winterStart = {2022: "2022-09-19", 2023: "2023-09-18"};
    const summerStart = {2023: "2023-02-06", 2024: "2024-02-05"};
    const dateWeek = getWeekNumber(date);
    const year = date.getFullYear();
    const winterStartWeek = getWeekNumber(new Date(winterStart[year]));
    const summerStartWeek = getWeekNumber(new Date(summerStart[year]));
    const relativeWinterWeek = dateWeek - winterStartWeek + 1;
    const relativeSummerWeek = dateWeek - summerStartWeek + 1;
