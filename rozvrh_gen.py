...
class ScheduleGenerator:
    def __init__(self, data):
        ...
        for i, x in enumerate(self.data):
            if x != self.get_first_of_lessons(x):
                self.problem.addConstraint(lambda a, b: a == b, [i, self.data.index(self.get_first_of_lessons(x))])

    def every_class_type_only_once(self):
        all_classes = [[] for _ in range(len(self.hours_list))]
        for i, j in enumerate(self.data):
            if j == self.get_first_of_lessons(j):
    ...
    def get_first_of_lessons(self, lesson):
        for x in self.data:
    ...
    def every_hour_only_once(self):
        ...
        for j in self.data:
            i = self.data.index(self.get_first_of_lessons(j))
    ...
    def add_constraint(self, max_sum, days, after, before, weighted=False):
        weights = [min(j["to"] - 1, before) - max(j["from"], after) + 1 for i, j in enumerate(self.data) if
                   j["day"] in days and j["from"] <= before and after <= j["to"] - 1]
        if len(weights) == 0:
            return
        if not weighted:
            weights = None
        self.problem.addConstraint(MaxSumConstraint(max_sum, weights), [i for i, j in enumerate(self.data) if
                                                                        j["day"] in days and j[
                                                                            "from"] <= before and after <= j["to"] - 1])
    ...
    def generate_schedule(self, id_):
        went_over = 0
        for x in self.problem.getSolutionIter():
            if went_over == id_:
                out = [0] * len(self.data)
